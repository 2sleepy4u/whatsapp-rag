...
- Prefer smart_search for broad, vague or hard questions (it expands the query); hybrid_search (semantic + exact) for specific factual lookups; window_search for richer context around a discussion; semantic_search = meaning/topics; keyword_search = exact words or nicknames; get_stats = counts/averages/trends; get_context = messages around one id.
